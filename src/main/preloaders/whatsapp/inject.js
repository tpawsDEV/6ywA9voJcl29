...
const Inject = async () => {
  ...
  if (tester) {
    const moduleRaidStr = () => {
      ...
      eval('var moduleRaid = ' + moduleRaidS);
      ...
      window.Store.ChatState = window.mR.findModule(
      ...
      window.Store.ConversationMsgs =
        window.mR.findModule('loadEarlierMsgs')[0];
    ...
    const WWebJS = () => {
      ...
      window.WWebJS.prepareMessageButtons = (buttonsOptions) => {
        ...
        if (
          typeof buttonsOptions.useTemplateButtons === 'undefined' ||
          buttonsOptions.useTemplateButtons === null
        ) {
      ...
      window.WWebJS.sendMessage = async (chat, content, options = {}) => {
        ...
        // }
        if (options.buttons) {
          options.buttons.body = options.buttons.body.split('{nl}').join('\n');
          options.buttons.title = options.buttons.title
            .split('{nl}')
            .join('\n');
          options.buttons.footer = options.buttons.footer
            .split('{nl}')
            .join('\n');
        }
        if (content) {
          content = content.split('{nl}').join('\n');
        }
        if (options.caption) {
          options.caption = options.caption.split('{nl}').join('\n');
        }
        ...
        const message = {
          ...
          isNewMsg: true,
          type: Object.keys(attOptions).length ? 'image' : 'chat',
          ...ephemeralFields,
          // ...locationOptions,
          ...attOptions,
          // ...quotedMsgOptions,
        ...
        await (window.Store.SendMessage.addAndSendMsgToChat
          ...
          : window.Store.SendMessage.default.addAndSendMsgToChat(
              chat,
              message
            ));
    ...
    const wwpFunctions = () => {
      window.archiveChat = async (chatId) => {
      ...
      window.sendMessage = async (chatId, message, options) => {
        let chatWid;
        try {
          ...
          try {
            ...
            throw `Não foi possível enviar mensagem para: ${chatId}`;
          }
        }
        const chat = await window.Store.Chat.find(chatWid);
      ...
      window.formatNumber = (n) => {
        if (n.length > 12) return n.slice(0, 4) + n.slice(5);
      ...
      window.getNumberId = async (number) => {
      ...
      window.clearContacts = async (msg) => {
        try {
          const final = [];
          for (let index = 0; index < contacts.length; index++) {
            const contact = contacts[index];
            let phone = formatNumber(contact.phone) + '@c.us';
            try {
              const wid = await window.getNumberId(phone);
              if (wid) {
                final.push({
                  _id: contact._id,
                  phone: wid.user,
              ...
              phone = add9(formatNumber(contact.phone)) + '@c.us';
              try {
                const wid = await window.getNumberId(phone);
                if (wid) {
                  final.push({
                ...
                final.push({
                  _id: contact._id,
                  phone: add9(formatNumber(contact.phone)),
                  is_whatsapp: false,
                });
              }
            }
      ...
      window.getContacts = async () => {
        ...
        for (let index = 0; index < contacts.length; index++) {
          try {
            ...
            if (!contact.isMe && contact.isUser) {
              const chat = await window.WWebJS.getChat(contact.id._serialized);

              const lb = new Date(chat.t * 1000);
              var dd = String(lb.getDate()).padStart(2, '0');
              var mm = String(lb.getMonth() + 1).padStart(2, '0'); //January is 0!
              var yyyy = lb.getFullYear();
              ...
              final.push({
                ...
                phone: contact.userid,
                last_buy: lastBuy.replace('NaN/NaN/NaN', '01/01/1969'),
                lastBuyFormat: 'd/m/Y',
                buys: 1,
              });
        ...
        return { contacts: final, groups: groups };
      };

      window.onAddMessageEvent = (msg) => {
        const eventMsgCreate = new Event('message_create', msg);
        document.dispatchEvent(eventMsgCreate);

        if (msg.id.fromMe) return;
        const eventMsg = new Event('message', msg);
    ...
    document.addEventListener('message_create', function (event) {
      // SE EU QUISER FAZER ALGO COM AS MENSAGENS RECEBIDAS, POSSO RODAR AQUI
    });
    ...
    setTimeout(() => {
      console.log('moduleRaidStr');
      var script = document.createElement('script');
      script.id = 'moduleRaidStr';
      script.textContent = `const moduleRaidStr = ${moduleRaidStr.toString()};moduleRaidStr()`;
    ...
    setTimeout(() => {
      ...
      script2.id = 'WWebJS';
      script2.textContent = `const WWebJS = ${WWebJS.toString()};WWebJS()`;
      document.head.appendChild(script2);
    }, 4000);

    setTimeout(() => {
      console.log('wwpFunctions');
      var script3 = document.createElement('script');
      script3.id = 'wwpFunctions';
      script3.textContent = `const wwpFunctions = ${wwpFunctions.toString()};wwpFunctions()`;
